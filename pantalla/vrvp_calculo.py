...
def calcular_vrvp():
    try:
        ...
        url = f'https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval={temporalidad}&limit={lookback_period}'
        ...
        print(f"Error en calcular_vrvp: {str(e)}")
        raise
...
if __name__ == "__main__":
    try:
        ...
        print("Resultados del cálculo de VRVP:")
        print(f"POC Precio: {resultado['poc_precio']:.2f}")
        print(f"VA Superior: {resultado['va_superior']:.2f}")
        print(f"VA Inferior: {resultado['va_inferior']:.2f}")
        print(f"Volumen Total: {resultado['volumen_total']:.2f}")
        print(f"Volumen VA: {resultado['volumen_va']:.2f}")
    except Exception as e:
        print(f"Error: {str(e)}")
