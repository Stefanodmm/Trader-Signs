    ...
    def update_info(self):
        ...
        for signal in self.active_signals:
            if signal == "RSI":
                ...
                vrvp_data = calcular_vrvp()  # Llamar a la función calcular_vrvp
                
                # Lógica para determinar la posición del precio en relación con los niveles
                if vrvp_data['poc_precio'] > vrvp_data['va_superior']:
                    # Calcular la posición relativa por encima de la banda superior
                    position_value = 10 + min(1, (vrvp_data['poc_precio'] - vrvp_data['va_superior']) / (vrvp_data['poc_precio'] - vrvp_data['va_superior'] + 1))
                elif vrvp_data['poc_precio'] == vrvp_data['va_superior']:
                    position_value = 10  # En el precio de la banda superior
                elif vrvp_data['poc_precio'] == vrvp_data['va_inferior']:
                    position_value = -10  # En el precio de la banda inferior
                elif vrvp_data['poc_precio'] < vrvp_data['va_inferior']:
                    # Calcular la posición relativa por debajo de la banda inferior
                    position_value = -10 - min(1, (vrvp_data['va_inferior'] - vrvp_data['poc_precio']) / (vrvp_data['va_inferior'] - vrvp_data['poc_precio'] + 1))
                else:
                    # Calcular la posición relativa entre las bandas
                    position_value = int((vrvp_data['poc_precio'] - vrvp_data['va_inferior']) / (vrvp_data['va_superior'] - vrvp_data['va_inferior']) * 20) - 10

                info_text += f"Posición VRVP: {position_value} | "  # Mostrar el valor de posición
                ...
                if position_value >= 10:  # Vender si el valor es 10 o 11
                    ...
                    recomendaciones.append("Mantener")
                    recomendaciones_individuales.append("VRVP recomienda: Mantener")
        ...
        # Obtener la configuración de espera inteligente
        with open(config_file) as f:
            config = json.load(f)
        espera_inteligente = config.get("VRVP", {}).get("espera_inteligente", 0)

        # Lógica para esperar hasta el final de la vela si se recomienda comprar o vender
        if espera_inteligente == 1 and (recomendacion_general == "Comprar" or recomendacion_general == "Vender"):
            self.timer.stop()  # Detener el temporizador
            self.timer.start(self.calcular_tiempo_hasta_siguiente_vela())  # Esperar hasta el inicio de la siguiente vela
            return  # Salir de la función para no actualizar inmediatamente

    def calcular_tiempo_hasta_siguiente_vela(self):
        # Calcular el tiempo restante hasta el inicio de la siguiente vela
        current_time = QtCore.QDateTime.currentDateTime()
        next_candle_time = current_time.addSecs(3600 - (current_time.time().minute() * 60 + current_time.time().second()))  # Para temporalidad de 1h
        return int(current_time.secsTo(next_candle_time) * 1000)  # Retornar el tiempo en milisegundos
