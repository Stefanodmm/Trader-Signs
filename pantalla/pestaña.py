...
class InfoWindow(QtWidgets.QWidget):
    ...
    def update_info(self):
        ...
        recomendaciones = []
        closes = []  # Inicializar la lista de precios de cierre

        # Evaluar cada señal activa
        for signal in self.active_signals:
            if signal == "RSI":
                rsi = self.calcular_rsi()
                ...
                macd, signal_line = self.calcular_macd()
                info_text += f"MACD: {macd:.2f} | "
                if macd > signal_line:
                    recomendaciones.append("Comprar")
                ...
                media_movil, upper_band, lower_band, closes = self.calcular_bollinger()  # Obtener closes
                info_text += f"B.Bolinger: {media_movil:.2f} | "
                ...
                # Aquí puedes agregar lógica para recomendaciones basadas en VRVP
                # Por ejemplo:
        ...
        if recomendaciones:
            info_text += f"\nRecomendaciones: {', '.join(set(recomendaciones))}"
        else:
            info_text += "\nSin recomendaciones."
    ...
    def calcular_bollinger(self):
        # Obtener datos históricos de Binance
        url = f'https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1d&limit=20'
        response = requests.get(url)
        ohlcv = response.json()
        closes = [float(x[4]) for x in ohlcv]  # Precios de cierre

        # Calcular Bandas de Bollinger
        periodo = 20  # Por ejemplo
        desviacion = 2  # Por ejemplo
        ...
        return media_movil, upper_band, lower_band, closes  # Devolver closes
